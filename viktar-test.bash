...
copyTestFiles()
{
    ...
    ${JPROG} -c -f bin_files.viktar *.bin
...
testCRCErrors()
{
    ...
    CORE_DUMP=$?
    coreDumpMessage ${CORE_DUMP} "testing extract from corrupt crc file 1"
    if [ ${CORE_DUMP} -eq 0 ]
    then
        ((POINTS+=10))
        echo -e "\tNice... Exit value from bad crc is 0"
    else
        echo "Exit value from a bad crc should be 0"
        echo ">>> Continuing - missing 10 points"
        CLEANUP=0
    fi
    ...
    if [ $? -ne 0 ]
    then
        echo ">>> FAILED bad crc in archive file 1"
        echo ">>> "
        echo "Your output:"
        ...
        ((POINTS+=10))
        echo -e "\tbad crc test 1.1 is good\n\tPOINTS=${POINTS}"
    fi
    if [ ! -e 5-s.txt ]
    then
        ...
        ((POINTS+=10))
        echo -e "\tbad crc test 1.2 is good\n\tPOINTS=${POINTS}"
    fi

    { timeout ${TOS} ${TO} bash -c "exec ${SPROG} -x < ${CORRUPT_FILE2} > corruptTest2.sout 2> corruptTest2.serr" ; }
    CORE_DUMP=$?
    coreDumpMessage ${CORE_DUMP} "testing extract from corrupt crc file 2"
    if [ ${CORE_DUMP} -eq 0 ]
    then
        ((POINTS+=10))
        echo -e "\tNice... Exit value from bad crc is 0"
    ...
    if [ $? -ne 0 ]
    then
        echo ">>> FAILED bad crc in archive file 2"
        echo ">>> "
        echo "Your output:"
        ...
        ((POINTS+=10))
        echo -e "\tbad crc test 2.1 is good\n\tPOINTS=${POINTS}"
    fi
    if [ ! -e 6-s.txt ]
    then
        echo ">>> You did not extract the corrupt file"
        ...
        ((POINTS+=10))
        echo -e "\tbad crc test 2.2 is good\n\tPOINTS=${POINTS}"
    fi

    echo "** Done with Testing bad viktar crc value."
    echo "*** Points so far ${POINTS} of ${TOTAL_POINTS}"
}
...
cleanTestFiles()
{
    ...
    then
        ...
        rm -f valgrindTest?.err WARN.err
